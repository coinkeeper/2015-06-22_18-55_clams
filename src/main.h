...
public:
    ...
    unsigned int GetSerializeSize(int nType, int nVersion) const {
        return ::GetSerializeSize(VARINT(nHeight*2+(fCoinBase ? 1 : 0)+(fCoinStake ? 1 : 0), nType, nVersion) +
    ...
    void Serialize(Stream &s, int nType, int nVersion) const {
        ::Serialize(s, VARINT(nHeight*2+(fCoinBase ? 1 : 0)+(fCoinStake ? 1 : 0), nType, nVersion);
        if (nHeight > 0)
...
public:
    ...
    unsigned int GetSerializeSize(int nType, int nVersion) const {
  	...
  	// update may be required for coinstake
